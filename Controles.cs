...
internal sealed unsafe class Controles
{
    ...
    internal Controles(HWND padre, float escala)
    {
        _padre = padre;
        _escala = escala;
        _letra = Letra(escala);
    ...
    /// <summary>Deja una plantilla puesta. Es lo que hace que abrir con una desde la consola ensene lo mismo que si la hubieras tecleado.</summary>
    internal void Escribe(string plantilla)
    {
        fixed (char* t = plantilla)
        {
            PInvoke.SetWindowText(_hijos[Plantilla], new PCWSTR(t));
        }
    }
    ...
    internal void Coloca(float ancho, float escala)
    {
        ...
        int libre = (int)ancho - margen * 2 - botones * 2 - margen * 2;
        int plantilla = (int)(libre * 0.52f);
        int caja = (libre - plantilla - margen * 2) / 2;
