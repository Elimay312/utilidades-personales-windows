...
- **Las cuentas se gestionan en la configuración (fase 12.3).** Sección «Cuentas de Google»: una tarjeta fija para conectar, añadir o reconectar (`GoogleSync::Connect` decide: la primera cuenta sin permiso, o una nueva), y debajo una tarjeta por cuenta con **Quitar** (`GoogleSync::Forget`, que borra su token y todo lo suyo de la caché, después de preguntar). La ventana crece con el número de cuentas (hasta 6). El menú de la bandeja agrupa el calendario por defecto por cuenta y la barra lateral separa los grupos con una línea.
- **La sincronización recorre todas las cuentas (fase 12.2).** Un `GoogleAuth` por cuenta. En cada pasada se pide la cabecera de cada una (la que no responde se salta y avisa, sin parar a las demás), se sube la cola entera —cada fila con la cuenta de su calendario— y se baja cuenta por cuenta. Mover un evento entre calendarios de cuentas distintas es borrarlo en la de origen y crearlo en la de destino: el `move` de Google no cruza cuentas. `ApplyEvent` solo empareja eventos dentro de la misma cuenta, así que un calendario compartido en dos cuentas sale dos veces, una por cuenta, cada una con su interruptor. Un calendario principal de otra cuenta no pasa a ser el predeterminado si ya hay uno.
