...
- **Una regla de repetición se despliega** (fase 6) con `OccursOn` en `src/core/recurrence.h`: FREQ DAILY/WEEKLY/MONTHLY/YEARLY con INTERVAL, BYDAY de días sueltos, COUNT y UNTIL. Lo que no entiende (BYDAY=1TU, BYMONTHDAY...) se queda en su primer día. Desde la fase 8.1 la columna `recurrence` guarda la lista entera de Google, una línea por línea, y `OccursOn` salta los días de sus EXDATE.
- **Una ocurrencia que Google aparta de su serie** (movida, editada o cancelada en la web) es una fila propia con `series_id` (el id **en Google** de la serie, no nuestro uid, porque es lo que llega y la serie puede venir en otra página) y `original_day`. Una cancelada se guarda como lápida (`deleted_at`) y no se borra. Los tres sitios que despliegan una serie (`ItemsForDay`, `DueReminders`, `DotsForRange`) se saltan los días apartados; la serie se identifica como `COALESCE(remote_id, uid sin guiones)`, que es el id con el que sube (`EventIdFor`).
- Reglas del parser:
