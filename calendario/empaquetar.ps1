<#
    Empaqueta Agenda en un único instalador: build\release\Instalar-Agenda.exe.

    Compila el preset release (Agenda.exe y, con él dentro, el instalador) y dice dónde quedó
    y cuánto pesa.
...
cmake --build --preset release --target agenda_installer
if ($LASTEXITCODE -ne 0) { throw 'Falló la compilación del instalador.' }

$instalador = Get-Item 'build\release\Instalar-Agenda.exe'
