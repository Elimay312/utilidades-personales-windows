<#
    Prepara el entorno de desarrollo y dependencias para Dock en Windows.
...
Write-Host "Comprobando entorno de .NET 10 para Dock..." -ForegroundColor Cyan
...
if (Test-Path 'Dock.csproj') {
    Write-Host "Restaurando paquetes NuGet y generadores de código..." -ForegroundColor Cyan
    & $dotnetExe restore Dock.csproj
    if ($LASTEXITCODE -ne 0) {
        throw "Fallo al restaurar los paquetes NuGet de Dock.csproj."
    }
