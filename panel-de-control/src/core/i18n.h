...
// the place it is drawn: T(L"Próximamente", L"Coming soon"). A table of ids in another file
// would make every call site a lookup to read, for two languages that are never going to be
// twenty. Copied from calendario/src/core/i18n.h.
...
// Set on the interface thread at start. Atomic because the system worker (phase 3 on) words its
// failure messages on its own thread.
